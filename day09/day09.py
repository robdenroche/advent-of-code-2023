...
# true if a value in the list is not zero
def not_zero(hist: list) -> bool:
    for i in hist:
        if i != 0:
            return True
...
# returns a list of the differences in hist
def get_diffs(hist: list) -> list:
    assert len(hist) > 1

    diffs = list()

    # start at 1 so can do i - 1
    for i in range(1, len(hist)):
        diffs.append(hist[i] - hist[i - 1])

    return diffs


def find_next(hist: list) -> int:

    # hist diffs will store the lists of differences
    # with the most recent at position 0
    hist_diffs = list()
    hist_diffs.append(hist)

    while not_zero(hist_diffs[0]):
        hist_diffs.insert(0, get_diffs(hist_diffs[0]))

    # sum up all the last values in the hist diff lists
    next = 0
    for i in range(1, len(hist_diffs)):
        next += hist_diffs[i][-1]

    return next


# running sum of next values
sum = 0
...
def find_previous(hist: list) -> int:
    ...
    # subtract all the first values in the hist diff lists
    next = 0
    for i in range(1, len(hist_diffs)):
        next = hist_diffs[i][0] - next
