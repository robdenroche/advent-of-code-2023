...
beginning of the race holding down the button, the boat's speed increases by
...
end of the race. Hold the button for 1 millisecond at the start of the race.
Then, the boat will travel at a speed of 1 millimeter per millisecond for 6
...
button for 7 milliseconds. That's the entire duration of the race. You never
let go of the button. The boat can't move until you let go of the button.
Please make sure you let go of the button so the boat gets to move. 0
millimeters. Since the current record for this race is 9 millimeters, there are
actually 4 different ways you could win: you could hold the button for 2, 3, 4,
...
with open("input.txt") as fh:
    for line in fh:
        ...
        if "Time:" in line:
            times = str(line.split("Time: ")[1]).split()
        elif "Distance:" in line:
            dists = str(line.split("Distance: ")[1]).split()

# running multiplication of winning durations
mult = 1

for i in range(0, len(times)):
    # count how many durations win
    win_count = 0
    for dur in range(0, int(times[i]) + 1):  # can hold it for the whole time
        dist = dur * (int(times[i]) - dur)
        if dist > int(dists[i]):
            win_count += 1
    assert win_count > 0    # don't want to zero out our mult
    mult *= win_count
...
Now, you have to figure out how many ways
there are to win this single race. In this example, the race lasts for 71530
milliseconds and the record distance you need to beat is 940200 millimeters.
You could hold the button anywhere from 14 to 71516 milliseconds and beat the
...
with open("input.txt") as fh:
    for line in fh:
        ...
        if "Time:" in line:
            times.append(str(line.split("Time: ")[1]).replace(" ", ""))
        elif "Distance:" in line:
            dists.append(str(line.split("Distance: ")[1]).replace(" ", ""))
...
for i in range(0, len(times)):
    # count how many durations win
    win_count = 0
    for dur in tqdm(range(0, int(times[i]) + 1)):
        # +1 since we can hold it for the whole time
        dist = dur * (int(times[i]) - dur)
        if dist > int(dists[i]):
            win_count += 1
    assert win_count > 0    # don't want to zero out our mult
...
print(mult)  # 39132886 - brute force, but only took 15 seconds
